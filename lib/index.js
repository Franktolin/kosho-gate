...
function buildFrpcToml(cfg) {
  ...
  L.push(`transport.useCompression = true`);
  L.push("");
  return L.join("\n");
}
...
// ---------------------------------------------------------------------------
// Open settings / agent-preset / credential RPCs to trusted-host clients
// (host plugin runs from src on source launches, from lib on installs).
const PRIVILEGED_RE = /^[ \t]*["'](?:settings|agentPreset|credentials)\.[^"']*["'],?\s*\n/gm;
// Force the settings scope to host persistence on every client bundle.
const PERSISTENCE_RE = /connection\.isLoopback \? ["']host["'] : ["']memory["']/;

const REMOTE_PATCHES = [
  {
    file: "node_modules/@deepseek-ai/dsh-client-connection/lib/index.js",
    srcFile: "packages/client/connection/src/index.ts",
    apply: (text) => text.replace(PRIVILEGED_RE, "")
  },
  {
    file: "node_modules/@deepseek-ai/dsh-client-ui-settings/lib/client.js",
    srcFile: "packages/client/ui-settings/src/client/settings-scope.ts",
  ...
  },
  {
    ...
    file: "node_modules/@deepseek-ai/dsh-client-ui-settings-models/lib/client.js",
    srcFile: "packages/client/ui-settings-models/src/client/index.ts",
    apply: (text) => text.replace(PERSISTENCE_RE, '"host"')
  }
];
