window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      remoteNoDshToken: "远端免 token",
      remoteNoDshTokenDesc: "局域网与 frpc 公网地址（trustedHosts 内）访问时跳过 dsh 的一次性 token URL",
      ...
      frpcPoolCount: "本地连接池 (poolCount)",
      frpcPoolCountHint: "并发转发连接数（约等于浏览器并发，建议 4~8，过高无益）",
      frpcCompression: "启用 frpc 压缩 (snappy)",
      frpcCompressionHint: "网关已 gzip 文本，默认关闭避免双层压缩",
      frpcStatus: "运行状态",
      frpcDisabled: "未启用",
      frpcRunning: "运行中",
      frpcNotRunning: "未运行",
    ...
    var en = {
      ...
      remoteNoDshToken: "No token for remote",
      remoteNoDshTokenDesc: "Skip dsh's one-time token URL for LAN & frpc public addresses (inside trustedHosts)",
      ...
      frpcPoolCount: "Local connection pool (poolCount)",
      frpcPoolCountHint: "Concurrent forwarding connections (~browser concurrency; 4-8 recommended; higher is wasteful)",
      frpcCompression: "Enable frpc compression (snappy)",
      frpcCompressionHint: "Gateway already gzips text; keep off to avoid double compression",
      frpcStatus: "Status",
    ...
    function LanGateSection(props) {
      ...
      var remoteNoDshToken = value.remoteNoDshToken !== false;
      ...
      var poolCount = value.frpcPoolCount == null ? "" : String(value.frpcPoolCount);
      var frpcCompression = value.frpcCompression === true;
      ...
      return createElement("div", { style: sectionStyle },
        ...
        createElement(Card, { heading: t("remoteTitle") },
          ...
          createElement(ToggleRow, {
            checked: remoteNoDshToken,
            disabled: !consented,
            onChange: function (v) { set("remoteNoDshToken", v); },
            title: t("remoteNoDshToken"),
            desc: t("remoteNoDshTokenDesc"),
            style: { padding: "2px 0" }
          })
        ),
        ...
        createElement(Card, { heading: t("frpcTitle") },
          ...
          createElement(Field, { label: t("frpcPoolCount"), desc: t("frpcPoolCountHint") },
            createElement("input", { value: poolCount, inputMode: "numeric", placeholder: "6", onChange: function (e) { var n = Number(e.target.value); set("frpcPoolCount", Number.isFinite(n) ? Math.max(1, n) : 6); }, style: textInputStyle })
          ),
          createElement(ToggleRow, {
            checked: frpcCompression,
            onChange: function (v) { set("frpcCompression", v); },
            title: t("frpcCompression"),
            desc: t("frpcCompressionHint"),
            style: { padding: "2px 0" }
          })
        ),
